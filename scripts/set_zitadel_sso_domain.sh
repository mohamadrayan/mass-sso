...
sed -i \
  -e "s/ZITADEL_FIRSTINSTANCE_ORG_HUMAN_PASSWORDCHANGEREQUIRED:.*/ZITADEL_FIRSTINSTANCE_ORG_HUMAN_PASSWORDCHANGEREQUIRED: true/" \
  docker-compose.yml

if ! grep -q 'ZITADEL_FIRSTINSTANCE_ORG_HUMAN_USERNAME' docker-compose.yml; then
  sed -i '/ZITADEL_FIRSTINSTANCE_ORG_HUMAN_PASSWORDCHANGEREQUIRED: true/a\
